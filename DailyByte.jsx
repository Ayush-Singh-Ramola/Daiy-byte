...
const storyBySlug = (slug) => STORIES.find((s) => s.slug === slug);
...
/* ============================================================
   PUBLIC PAGES
   ============================================================ */
function HomePage({ onOpenStory, onNav, onSubscribed }) {
...
function CategoryPage({ category, onOpenStory, onNav }) {
...
function StoryPage({ slug, onOpenStory, onNav }) {
...
function IssueReaderPage({ slug, onOpenStory, onNav }) {
  ...
  return (
    <div className="max-w-[1400px] mx-auto px-5 sm:px-8 py-10">
      ...
      {issue.sections.map((section, si) => {
        const stories = section.slugs.map(storyBySlug).filter(Boolean);
          ...
          <div key={section.name} className="mt-16">
            ...
            {si === 0 ? (
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
                <div className="lg:col-span-7">
                  <HeroStory story={stories[0]} onOpen={onOpenStory} />
                </div>
                <div className="lg:col-span-5">
                  {stories.slice(1).map((s) => <CompactStory key={s.slug} story={s} onOpen={onOpenStory} />)}
                </div>
              </div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-8">
                {stories.map((s) => <StandardStory key={s.slug} story={s} onOpen={onOpenStory} />)}
              </div>
            )}
          </div>
        );
...
function SearchPage({ onOpenStory, onNav }) {
  ...
  return (
    <AdminShell page="admin-stories" onNav={onNav}>
      <div className="flex items-center justify-between mb-8">
        ...
        <button style={{ ...sans, background: COLORS.ink, color: COLORS.paper }} className="text-[13px] px-4 py-2.5 flex items-center gap-2">
...
export default function App() {
  ...
  switch (route.name) {
    case "home":
      page = <HomePage onOpenStory={openStory} onNav={nav} onSubscribed={handleSubscribed} />;
      break;
    case "category":
      page = <CategoryPage category={route.param} onOpenStory={openStory} onNav={nav} />;
      break;
    case "story":
      page = <StoryPage slug={route.param} onOpenStory={openStory} onNav={nav} />;
      break;
    case "issues":
      page = <IssuesPage onOpenIssue={openIssue} onNav={nav} />;
      break;
    case "issue":
      page = <IssueReaderPage slug={route.param} onOpenStory={openStory} onNav={nav} />;
      break;
    case "search":
      page = <SearchPage onOpenStory={openStory} onNav={nav} />;
      break;
    case "subscribed":
